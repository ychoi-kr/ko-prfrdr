...
KW_NTAv = "정도|메서드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[변인함]수|분위수|순서|마이크로서비스|클래스|단어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|에포크|배치|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|폰트|분포|변화|기회"
