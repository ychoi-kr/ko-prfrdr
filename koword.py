...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|공간|물건|학년|기능|[능출]력|연령|프로그램|명산|걸음|전략|칼럼|입력|블록|그룹|[품항]목|논문|방법|형상|성|특성|솔루션|커넥션|애플리케이션|현실|직업|앱|영역|차원|도메인|요인|타입|점|규정|[시지]점|수준|알고리즘|군집|품질|층|토큰|스텝|시스템|패턴|팀|상품|토픽|모형"
KW_NTAfl = "채널|모델|모듈|행렬|메일|스타일|튜플"
KW_NTAf = joinseq(KW_NTAfc, KW_NTAfl)
KW_NTAv = "근거|정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|의사|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|서비스|클래스|[단언]어|레이어|소프트웨어|하드웨어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|[고기물신액옥자형]체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|번호|비밀번호|변화|기회"
...
KW_NAOf = "생각|보간|[가수증]감|동결|곱|[가제]공|[건입출]국|공격|[연체]결|변경|[총포]괄|집권|[접출]근|송금|지급|가늠|기능|판단|[도전]달|[담배]당|[가기이작]동|[납취획]득|누락|[노]력|[관마수제훈]련|[기수]록|[발변설연작제조증]명|[괄주지]목|함몰|질문|고민|위반|[개선]발|체벌|식별|[반중]복|구분|준비|계산|[손연향]상|발생|[분해]석|개선|[건개배연증해]설|[구생작완형]성|[계상접]속|[방소]송|[기저]술|[연학]습|[송수]신|송수신|임신|[장파]악|[고제]안|[계예해]약|선언|[수성영작]업|[구시재]연|오염|[반운촬투]영|[사이작적통포활]용|훈육|[기지]원|불응|방일|통일|[기도수투]입|[수]?출입|[동시조]작|[긴등성저출]장|경쟁|산적|[발운회]전|조절|[걱검결설수지추측한]정|참조|방중|검증|감지|부진|선언|[개선수확]정|[도장부집탈]착|칭찬|제창|구축|[도산수연인제진창추]출|[계예]측|강탈|[간선채]택|[면유입재]학|국한|방한|분할|포함|[결부취]합|[동발비서시수운연진]행|[재]?실행|[구실재표]현|[반변]환|계획"
...
KW_NAFv = "다운로드|릴리스|마사지|이미지|업로드|[언]?마운트|업데이트|임포트|카운트|테스트|트리거|팔로우|폴로|플레이"
