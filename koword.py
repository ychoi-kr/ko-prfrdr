...
## Thing
### Abstract
KW_NTAf = "걸음|명산|모델|모형|모듈|방법|블록|스텝|시스템|알고리즘|애플리케이션|앱|입력|전략|점|차원|채널|출력|층|토큰|토픽|튜플|특성|프로그램|함수|행렬"
KW_NTAv = "가중치|글자|단어|데이터|리뷰|메모리|변화|배치|벡터|시류|요소|인수|자체|절차|정도|차이|처리|축소|코드|이벤트|폰트|학습 데이터|형태"
