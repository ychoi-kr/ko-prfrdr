...
KW_NAOh = "[0-9]+차원화|가시화|간소화|강화|경량화|공산화|구간화|대형화|도커화|디지털화|모듈화|미화|무료화|민주화|복호화|소형화|수치화|세분화|시각화|암호화|유료화|융화|인스턴스화|일반화|중앙화|첨예화|초기화|최[신적]화|캡슐화|파편화|패턴화|특[수]?화|현[실행]화|형상화"
...
KW_NABf = "비난|감동|처단|할당|[입출]력|주목|처방|[처]?벌|축복|상속|전송|교육|환영|구원|위임|대입|연장|판정|강종|칭찬|추천|부탁|선택|심판|반환|시험"
...
KW_NTAv = "정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|클래스|단어|레이어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|에포크|배치|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|변화|기회"
