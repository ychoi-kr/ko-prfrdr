...
KC_D = "[되된돼됐]"
KC_H = "하고[는자]?|하겠\\w+|하[는니다며여]|하도록|하려[고는]?|하[?]면|하였\\w+|하지[만]?|한 [것뒤적후]|한 다음|한다[고는니며지]?|할까|할[지]?[도]?|함[을]?|함이다|함입니다|합니다|해보[니다면]|해볼[까래]?|해[서요]?|했고[요]?|했는데[요]?|했다[만]?|했습니다[만]?|했으[니면][서]?|했을[지]?|했지[만]?[요]?"
