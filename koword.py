...
KW_NAOv = "[인증추참평]가|[제탈]거|[공소]개|[경방]과|[연촉]구|[대상연제]기|이야기|시도|[고]려|[완치]료|[관격수]리|[전후]?처리|마무리|연마|구매|소모|근무|[공거기]부|[감공반발수식조퇴]사|[감기축]소|[감준회]수|제시|제어|[기참]여|[긴소중필]요|[논유정합회]의|[재]?정의|복제|[유주차폐]지|대체|[성]취|[납배설]치|검토|실패|배포|저하|[분이]해|" + KW_NAOh 
...
## Thing(무정물)
### Abstract
KW_NTAf = "값|관측값|구간|채널|학년|모델|모듈|[능출]력|연령|프로그램|명산|걸음|전략|칼럼|입력|행렬|블록|그룹|[품항]목|논문|방법|성|특성|솔루션|애플리케이션|직업|앱|차원|도메인|요인|타입|점|[시지]점|수준|알고리즘|군집|층|토큰|스텝|시스템|패턴|팀|상품|튜플|토픽|모형"
KW_NTAv = "정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|클래스|단어|레이어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|관측치|에포크|배치|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|변화|기회"
