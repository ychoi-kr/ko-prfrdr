...
## Nouns - Status
KW_NSf = "[둔민]감|건강|간결|깔끔|[불]?가능|간단|근면|곤란|[장졸]렬|동일|[미병심]약|치열|[단온지]순|참신|[불]?성실|부실|복잡|부족|[부]?적절|공평|[명정]확"
KW_NSv = "유사|[자섬]세|상이|수요|[거장중]대|진부|[긴중필]요"
...
KW_Ff = "[A-Za-z0-9]*all|[A-Za-z0-9]*ell|[A-Za-z0-9]*earn|[A-Za-z0-9]*ime|[A-Za-z0-9]*one|[A-Z]*[L-NR]"  # f: has final consonant in Korean
KW_Fv = "[A-Za-z0-9]*ice|[A-Za-z0-9]*ocks|[A-Za-z0-9]*old|[A-Za-z0-9]*uy|[A-Za-z0-9]*dog|[A-Za-z0-9]*uire|[A-Z]*[A-KOPQS-Z]"  # v: ends with vowel (does not have final consonant) in Korean


## Nouns - Activity (can be verb with '-하다/-되다' or used in form of '~을/를 하다')
### Original (Korean + Hanja)
KW_NAOf = "생각|보간|[가수증]감|곱|[가제]공|[건입출]국|공격|[연체]결|변경|[총포]괄|집권|[접출]근|송금|가늠|기능|도달|[담배]당|[가이작]동|[납취획]득|누락|[노]력|[관마수제훈]련|[기수]록|[발변설연작제조증]명|괄목|함몰|질문|고민|[개선]발|체벌|[반중]복|구분|준비|계산|[손연향]상|발생|[분해]석|개선|[건개배연증해]설|[구생작완형]성|[계상접]속|[방소]송|[기저]술|[연학]습|임신|[장파]악|[고제]안|[예해]약|선언|[수성영작]업|[구시재]연|오염|[반운촬투]영|[사이작적통포활]용|훈육|[지]원|불응|통일|[도수투]입|[수]?출입|[동시조]작|[긴등성저출]장|경쟁|산적|[발운회]전|조절|[걱검결설수지추측한]정|참조|검증|부진|선언|[개선수확]정|[도장부집탈]착|칭찬|제창|구축|[도산수연인제진창추]출|[계예]측|강탈|[간선채]택|[면유입재]학|국한|분할|포함|[결부취]합|[동비서시수운연진]행|[재]?실행|[구실재표]현|[반변]환|계획"
KW_NAOh = "[0-9]+차원화|가[상속시]화|간소화|강화|경량화|고도화|공산화|구[간조]화|균일화|다중화|대형화|도커화|디지털화|모듈화|미화|무료화|민주화|\\w*변수화|복호화|소형화|수치화|세분화|시각화|암호화|유료화|융화|의무화|인스턴스화|일반화|중앙화|집약화|첨예화|초기화|최[신적]화|캡슐화|파편화|패턴화|특[수]?화|현[실행]화|해시화|형상화|효율화"
KW_NAOv = "[인증추참평]가|[제탈]거|[공소]개|[경방]과|[복연촉]구|[경대상연제]기|이야기|[시유]도|[고]려|[완치]료|[관격분수]리|[전후]?처리|마무리|연마|구매|소모|근무|[공거기]부|[감공반발수식조퇴]사|[감기축]소|[감준회]수|[실제출]시|제어|[기참]여|[긴소중필]요|[논유정주합회]의|[재]?정의|복제|[유주차폐]지|대체|[성]취|[납배설]치|검토|돌파|[부실]패|배포|발표|[저전통]화|[분이]해|" + KW_NAOh 
...
KW_NPAv = "수집가|애호가|프로|아마추어|글쓴이|엮은이|지은이|감[독시]자|강[연의]자|관[계련찰]자|당[사직]자|[사이]용자|소유자|반[려역]자|발[제표]자|응시자|[원]작자|협력자|[가피]해자|팬"
...
## Thing(무정물)
### Abstract
KW_NTAf = "관측값|[공구]간|물건|채널|학년|모델|모듈|[능출]력|연령|프로그램|명산|걸음|전략|칼럼|입력|행렬|블록|그룹|[품항]목|논문|방법|성|특성|솔루션|애플리케이션|현실|직업|앱|차원|도메인|요인|메일|스타일|타입|점|규정|[시지]점|수준|알고리즘|군집|층|토큰|스텝|시스템|패턴|팀|상품|튜플|토픽|모형"
KW_NTAv = "정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|서비스|클래스|[단언]어|레이어|소프트웨어|하드웨어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|변화|기회"
KW_NTA = joinseq(KW_NTAf, KW_NTAv)

### maChine(tool)
KW_NTCf = "로봇|툴"
KW_NTCv = "API|CPU|GPU|기계|비행기|생성기|판별기|항공기|웹[ ]?브라우저|로더|서버|장비|차|기차|자동차|컴퓨터|클라이언트"
...
KW_NTf = joinseq(KW_NTAf, KW_NTCf, KW_NTDf, KW_NTEf, KW_NTFf, KW_NTLf, KW_NTMf, KW_NTSf, KW_NTUf, KW_NTVf)
KW_NTv = joinseq(KW_NTAv, KW_NTCv, KW_NTDv, KW_NTEv, KW_NTFv, KW_NTLv, KW_NTMv, KW_NTSv, KW_NTUv, KW_NTVv)
