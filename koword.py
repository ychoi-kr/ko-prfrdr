...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|공간|물건|학년|[능출]력|연령|프로그램|테이블|명산|걸음|전략|칼럼|입력|블록|그룹|스트림|[품항]목|논문|방법|형상|성|창의성|특성|솔루션|커넥션|애플리케이션|[상지]식|현실|직업|앱|영역|차원|도메인|요인|타입|점|규정|[시지]점|수준|알고리즘|군집|품질|층|토큰|스텝|시스템|패턴|팀|상품|토픽|국민[ ]?평형|모형"
KW_NTAfl = "채널|모델|모듈|행렬|메일|스타일|튜플"
KW_NTAf = joinseq(KW_NTAfc, KW_NTAfl)
KW_NTAv = "근거|정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|의사|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|서비스|클래스|[단언]어|레이어|소프트웨어|하드웨어|범위|차이|글자|일자|범주|이미지|[시절]차|아키텍처|피처|[고기물신액옥자형]체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|번호|비밀번호|변화|기회"
