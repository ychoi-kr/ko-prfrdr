...
KW_NTAv = "선전 포고|[과근선]거|결과|[용정]도|가이드|메서드|스레드|코드|사용료|미래|[자재]료|시류|메모리|라이브러리|카테고리|리뷰|경영비|의사|[요축]소|[난변소인홍함]수|[극]?소수|부동소수점수|분위수|[순질]서|마이크로서비스|서비스|시퀀스|클래스|[단언]어|레이어|소프트웨어|아이디어|하드웨어|수요|범위|차이|글자|현재|일자|이미지|[시절]차|아키텍처|피처|[객물신옥형]체|가중치|관측치|배치|네트워크|에포크|태스크|세태|데이터|벡터|학습 데이터|[상형]태|세그먼트|이벤트|컴포넌트|크레이트|폰트|전자파|[분세점]포|[목좌]표|면허|번호|비밀번호|변화|기회"
