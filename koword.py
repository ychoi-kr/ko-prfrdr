...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|공간|물건|자금|학년|게놈|[능출]력|연령|프로그램|명산|걸음|목적|전략|칼럼|입력|기록|블록|그룹|스트림|별명|[품항]목|논문|방법|재산|형상|성|창의성|특성|솔루션|커넥션|애플리케이션|웹 애플리케이션|[상지]식|방안|직업|앱|영역|[내비]용|차원|웹|도메인|요인|타입|점|규정|[단시장지]점|수준|알고리즘|인터넷|군집|대책|층|명칭|토큰|스택|스텝|시스템|패턴|팀|상품|토픽|국민[ ]?평형|모형|힙"
...
### can Be(‘-화’)
KW_NTBf = "시각|구간|모듈|[대경]량|[병직]렬|[무유]료|그룹|일반|세분|공산|[가형]상|가속|캡슐|최신|현실|중앙|집약|차원|일원|효율|균일|최적|다중|계층|패턴|디지털|파편|현행|[대소]형"
...
KW_NTCf = "로봇|툴"
KW_NTCv = "API|CPU|GPU|기계|마이크로 프런트엔드|백엔드|비행기|생성기|판별기|항공기|침대|로더|서버|웹[ ]?서버|장비|브라우저|웹[ ]?브라우저|스토리지|[객기마열]?차|자동차|웹 워커|컴퓨터|클라이언트|프런트엔드"
...
### Language
KW_NTLf = "파이썬"
KW_NTLv = "독일어|북한어|스페인어|영어|이탈리아어|일본어|자바스크립트|중국어|터키어|한국어"
