...
KW_NSv = "무의미|유사|[자섬]세|간소|[거장중]대|[중필]요|분포|난해"
...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|물건|[자지]금|학년|게놈|성능|돈|[능출]력|연령|프로그램|걸음|목적|전략|칼럼|입력|기록|블록|그룹|스트림|화면|별명|[품항]목|[구논]문|방법|재산|명산|[세형]상|인생|성|창의성|[속특]성|데이터셋|솔루션|커넥션|애플리케이션|웹 애플리케이션|[공방상지형]식|방안|직업|앱|영역|[내비]용|차원|웹|마음|도메인|디자인|요인|타입|점|규정|[단시장지]점|수준|알고리즘|인터넷|군집|대책|층|명칭|토큰|스택|스텝|시스템|패턴|팀|상품|토픽|국민[ ]?평형|모형|힙"
...
KW_NTAv = "선전 포고|근거|결과|[용정]도|가이드|메서드|스레드|코드|사용료|미래|[자재]료|시류|메모리|라이브러리|카테고리|리뷰|경영비|의사|[요축]소|[난변소인홍함]수|[극]?소수|부동소수점수|분위수|[순질]서|마이크로서비스|서비스|시퀀스|클래스|[단언]어|레이어|소프트웨어|아이디어|하드웨어|수요|범위|차이|글자|일자|이미지|[시절]차|아키텍처|피처|[객물신옥형]체|가중치|관측치|배치|네트워크|에포크|태스크|세태|데이터|벡터|학습 데이터|[상형]태|세그먼트|이벤트|컴포넌트|크레이트|폰트|전자파|[분세점]포|[목좌]표|면허|번호|비밀번호|변화|기회"
