...
### Degree (정도)
KW_BD = '가장|그다지|더|많이|별로|약간|얼마간|자주|잘|조금|훨씬'
...
KW_NAOf = "생각|보간|[가수증]감|가공|곱|[건입출]국|공격|[연체]결|변경|[총포]괄|집권|접근|송금|가늠|기능|도달|[담배]당|[가이작]동|[납취획]득|누락|[노]력|[관마수제훈]련|[기수]록|[발변설연작제조증]명|괄목|함몰|질문|고민|[개선]발|체벌|[반중]복|준비|[손연향]상|[분해]석|개선|발생|[건개배연증해]설|[구생작완형]성|[계상]속|[연학]습|임신|[장파]악|[예해]약|[수성영작]업|[구시재]연|오염|[반운촬투]영|[사이작적통포활]용|훈육|[지]원|불응|[도수투]입|[수]?출입|[동시조]작|[긴등성저출]장|[발운회]전|조절|[걱검결설수지추측한]정|검증|부진|선언|[개선수]정|[도장부집탈]착|칭찬|제창|[도산수연인제진창추]출|[계예]측|[간선채]택|[면유입재]학|국한|분할|포함|[결부취]합|[전통]화|반환|[구실재표]현|[동비서시수운연진]행|[재]?실행"
KW_NAOh = "[0-9]+차원화|가시화|간소화|강화|경량화|공산화|구간화|대형화|도커화|디지털화|모듈화|미화|무료화|민주화|\\w*변수화|복호화|소형화|수치화|세분화|시각화|암호화|유료화|융화|인스턴스화|일반화|중앙화|첨예화|초기화|최[신적]화|캡슐화|파편화|패턴화|특[수]?화|현[실행]화|형상화"
KW_NAOv = "[인증추참평]가|[제탈]거|[공소]개|[경방]과|[연촉]구|[대상연제]기|이야기|[시유]도|[고]려|[완치]료|[관격수]리|[전후]?처리|마무리|연마|구매|소모|근무|[공거기]부|[감공반발수식조퇴]사|[감기축]소|[감준회]수|제시|제어|[기참]여|[긴소중필]요|[논유정합회]의|[재]?정의|복제|[유주차폐]지|대체|[성]취|[납배설]치|검토|실패|배포|저하|[분이]해|" + KW_NAOh 
KW_NAO = joinseq(KW_NAOf, KW_NAOv)

## can be Both of active(-하다) and passive(-받다)
KW_NABf = "비난|감동|처단|할당|[입출]력|주목|처방|[처]?벌|축복|상속|전송|제안|교육|환영|구원|[승확]인|위임|대입|연장|판정|강종|칭찬|추천|부탁|선택|심판|반환|시험"
...
KW_NPAv = "수집가|애호가|프로|아마추어|글쓴이|엮은이|지은이|감[독시]자|강[연의]자|관[계련찰]자|당[사직]자|[사이]용자|반[려역]자|발[제표]자|응시자|[원]작자|협력자|팬"
...
## Thing(무정물)
### Abstract
KW_NTAf = "관측값|구간|채널|학년|모델|모듈|[능출]력|연령|프로그램|명산|걸음|전략|칼럼|입력|행렬|블록|그룹|[품항]목|논문|방법|성|특성|솔루션|애플리케이션|직업|앱|차원|도메인|요인|타입|점|[시지]점|수준|알고리즘|군집|층|토큰|스텝|시스템|패턴|팀|상품|튜플|토픽|모형"
KW_NTAv = "정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|클래스|[단언]어|레이어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|관측치|에포크|배치|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|변화|기회"
...
KW_NTEv = "테슬라|발행사|회사|업소|엔비디아|이탈리아|한라|호주"
