...
KC_D = "[되된돼됐]"
KC_H = "하고[는자]?|하겠\\w+|하[는니다며여]|하도록|하려[고는]?|하[?]면|하여[야]?|하였\\w+|하지[만]?|한 [것뒤적후]|한 다음|한다[고는니며지]?|할|할까[요]?|할래|할지[도]?[요]?|함[을]?|함이다|함입니다|합니다|해보[니다면]|해볼[까래]?|해[서야요]?|했고[요]?|했기|했는데[요]?|했다[만]?|했습니다[만]?|했으[니면][서]?|했을[지]?|했지[만]?[요]?"
...
### maChine
KW_NTCf = "로봇|툴"
KW_NTCv = "CPU|GPU|기계|서버|장비|컴퓨터|클라이언트"
