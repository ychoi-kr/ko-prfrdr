...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|공간|물건|자금|학년|게놈|[능출]력|연령|프로그램|명산|걸음|목적|전략|칼럼|입력|블록|그룹|스트림|별명|[품항]목|논문|[동식]물|동식물|방법|형상|성|창의성|특성|솔루션|커넥션|애플리케이션|[상지]식|직업|앱|영역|비용|차원|웹|도메인|요인|타입|점|규정|[시지]점|수준|알고리즘|인터넷|군집|층|토큰|스텝|시스템|패턴|팀|상품|토픽|국민[ ]?평형|모형"
...
KW_NTAv = "근거|출입구|[용정]도|메서드|스레드|코드|사용료|시류|메모리|처리|카테고리|리뷰|의사|[요축]소|[난변소인함]수|부동소수점수|분위수|순서|마이크로서비스|서비스|클래스|[단언]어|레이어|소프트웨어|하드웨어|범위|차이|글자|일자|범주|이미지|[시절]차|아키텍처|피처|[고기물신액옥자형]체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|전자파|[분세점]포|번호|비밀번호|변화|기회"
...
KW_NTCf = "로봇|툴"
KW_NTCv = "API|CPU|GPU|기계|마이크로 프런트엔드|백엔드|비행기|생성기|판별기|항공기|침대|로더|서버|웹[ ]?서버|장비|브라우저|웹[ ]?브라우저|스토리지|차|기차|자동차|웹 워커|컴퓨터|클라이언트|프런트엔드"
...
KW_NAOf = "생각|보간|[가수증]감|동결|불구경|곱|[가제]공|[건입출]국|공격|[연체]결|[구변]경|[총포]괄|집권|[접출]근|송금|지급|가늠|기능|[차판]단|[도전]달|[담배해]당|[가기운이작]동|[납취획]득|누락|[노]력|[관마수제훈]련|[기수]록|소멸|[발변설소연작제조증]명|[괄주지]목|함몰|질문|고민|위반|[개선]발|체벌|식별|[반중]복|구분|계산|[손연향]상|발생|[분해]석|개선|[건개배연증해]설|[구생작완형]성|[계상접]속|[방소전]송|[기저]술|[연학]습|[발송수통]신|송수신|임신|[장파]악|[고보제]안|[계예해]약|선언|[수성영작]업|[구시재]연|오염|[반운촬투]영|[사응이작적통포허활]용|훈육|[기지]원|불응|방일|통일|[기도수투]입|[수]?출입|[동시조]작|[긴등성저출]장|경쟁|산적|[발운회]전|조절|[개걱검결선설수지추측한확]정|[마방]중|검증|감지|부진|[결매모수편]집|[도장부집탈]착|칭찬|제창|[구압]축|[도산수연인제진창추]출|[계예]측|[강이]탈|[간선채]택|[면유입재]학|국한|방한|분할|포함|[결부취]합|[동발비서시수운연진]행|[재]?실행|[구실재표]현|[반변]환|계획"
KW_NAOh = '|'.join(conjugate("강|미|융|특", '화') + conjugate(ks.KS_Az, '화') + conjugate(KW_NTB, '화'))
KW_NAOv = "[인증추참평]가|[제탈]거|[공소전]개|설계|[경방]과|[복연촉]구|[경대상연제표]기|이야기|[연증확]대|[시유]도|[고]려|신뢰|[완치]료|[관격분수]리|[전후]?처리|마무리|연마|구매|소모|근무|방미|[공거기]부|준비|[감공반발수식조퇴]사|[감기축]소|[감준회]수|[감실제출표]시|제어|[기참]여|[긴소중필]요|[논유정주합회]의|[재]?정의|투자|복제|참조|[유주차폐]지|[기탑]재|대체|[갈성수탈편]취|[납배설]치|검토|돌파|[부실]패|배포|발표|보호|[저전통]화|[분이]해|" + KW_NAOh 
...
KW_NPJf = "감독[관]?[님]?|강사님|과장[님]?|교수님|[국군도동시]민|기업인|대[리표]님|대통령[님]?|[사의]원|[사소의회]장[님]?|시인|작[곡사]?가님|직장인|고교생|[대중]?학생|초등학생|[수해]병|이사장|초대 회장|회사원|군인|경찰|동호인"
KW_NPJv = "프로그래머|간호사|[강검의판]사|교[사수]|대[리표]|작[곡사]?가|전문가|개발자|데이터[ ]?분석가"
