...
KW_NAOf = "생각|[가수증]감|가공|곱|[건입출]국|공격|[연체]결|[총포]괄|집권|접근|송금|가늠|[담배]당|[가이작]동|[납취획]득|[노]력|[관마수제훈]련|[기수]록|[발변설연작제조증]명|괄목|함몰|질문|고민|[개선]발|체벌|[반중]복|준비|[수조]사|[손연향]상|개선|발생|[건개배연증해]설|[구생작완형]성|[계상]속|[연학]습|임신|[장파]악|[예해]약|[수성영작]업|[구시재]연|오염|[반운촬투]영|[사이작적통포활]용|훈육|[지]원|불응|[승확]인|[도수투]입|[수]?출입|[동시]작|[긴등성저출]장|[발운회]전|조절|[걱검결설수지추측한]정|검증|부진|선언|[개선수]정|[도장부집탈]착|칭찬|제창|[도수연인제진창추]출|[계예]측|[간선채]택|[면유입재]학|국한|분할|포함|[결부취]합|[미전통]화|반환|[구실재표]현|[동비서시수운연진]행|[재]?실행"
...
KW_NAOv = "[인증추참평]가|[제탈]거|[공소]개|[경방]과|[연촉]구|[대상연제]기|이야기|시도|[고]려|[완치]료|[관격수]리|[전후]?처리|마무리|연마|소모|근무|[공거기]부|[감반발조]사|[감기축]소|[감준회]수|제시|제어|[기참]여|[긴소중필]요|[논유합회]의|복제|[유주차폐]지|대체|[성]취|[납배설]치|검토|실패|배포|저하|이해|" + KW_NAOh 
KW_NAO = joinseq(KW_NAOf, KW_NAOv)

## can be Both of active(-하다) and passive(-받다)
KW_NABf = "비난|감동|처단|할당|[입출]력|주목|처방|[처]?벌|축복|상속|전송|교육|구원|위임|대입|연장|판정|강종|칭찬|추천|부탁|선택|심판|반환|시험"
...
KW_NPJf = "감독[관]?[님]?|강사님|과장[님]?|교수님|[국군도동시]민|기업인|대[리표]님|대통령[님]?|[사의]원|[사소의회]장[님]?|시인|작[곡사]?가님|직장인|학생|[수해]병|초대 회장|회사원|군인|경찰|동호인"
...
KW_NPAv = "감독자|감시자|강[연의]자|관[계련찰]자|글쓴이|당[사직]자|[사이]용자|반[려역]자|발[제표]자|수집가|아마추어|엮은이|애호가|응시자|[원]작자|지은이|팬|프로|협력자"
...
KW_NTAv = "정도|메서드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[변인함]수|분위수|순서|마이크로서비스|클래스|단어|레이어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|에포크|배치|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|폰트|분포|변화|기회"
