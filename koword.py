...
## Nouns - Status
KW_NSf = "심각|[둔민]감|건강|간결|깔끔|들쑥날쑥|매끈|[불]?가능|간단|못마땅|가득|근면|곤란|부지런|[장졸]렬|지저분|[단온지]순|비슷|참신|[불]?성실|부실|[미병심]약|치열|편안|동일|부족|[부]?적절|착|공평|[명정]확"
...
## Thing(무정물)
### Abstract
KW_NTAfc = "관측값|물건|자금|학년|게놈|[능출]력|연령|프로그램|명산|걸음|목적|전략|칼럼|입력|기록|블록|그룹|스트림|화면|별명|[품항]목|논문|방법|재산|형상|성|창의성|[속특]성|솔루션|커넥션|애플리케이션|웹 애플리케이션|[상지]식|방안|직업|앱|영역|[내비]용|차원|웹|마음|도메인|디자인|요인|타입|점|규정|[단시장지]점|수준|알고리즘|인터넷|군집|대책|층|명칭|토큰|스택|스텝|시스템|패턴|팀|상품|토픽|국민[ ]?평형|모형|힙"
...
KW_NTAv = "근거|[용정]도|가이드|메서드|스레드|코드|사용료|시류|메모리|처리|라이브러리|카테고리|리뷰|의사|[요축]소|[난변소인함]수|[극]?소수|부동소수점수|분위수|[순질]서|마이크로서비스|서비스|클래스|[단언]어|레이어|소프트웨어|하드웨어|범위|차이|글자|일자|이미지|[시절]차|아키텍처|피처|[물신옥자형]체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|[상형]태|세그먼트|이벤트|컴포넌트|크레이트|폰트|전자파|[분세점]포|좌표|번호|비밀번호|변화|기회"
KW_NTA = joinseq(KW_NTAf, KW_NTAv)

### can Be(‘-화’)
KW_NTBf = "시각|구간|모듈|간략|[대경]량|[병직]렬|[무유]료|그룹|일반|세분|공산|[가형]상|활성|간소|[가고]속|캡슐|상식|최신|현실|중앙|집약|공용|차원|일원|효율|균일|복잡|최적|다중|계층|패턴|디지털|공통|파편|현행|[대소]형"
KW_NTBv = "체계|카탈로그|[동초]기|[극최]대|고도|의무|정보|[극최]소|독립변수|변수|특수|인스턴스|가시|해시|구조|민주|범주|내재|[고기액]체|수치|도커|[암복]호"
KW_NTB = joinseq(KW_NTBf, KW_NTBv)


### maChine, instrument, tool, furniture
KW_NTCf = "Babel|NPM|로봇|툴"
KW_NTCv = "API|CPU|GPU|Grunt|gulp|기계|마이크로 프런트엔드|백엔드|비행기|생성기|판별기|항공기|침대|로더|서버|웹[ ]?서버|장비|데이터베이스|브라우저|웹[ ]?브라우저|스토리지|[객기마열]?차|자동차|웹 워커|컴퓨터|클라이언트|태스크 러너|프런트엔드"
...
KW_NTJv = "영구|대내|진보|보수|도시|임시|[임한]시|대외|독자|[민자]주|향토"
...
KW_NTLAv = "루비|자바|자바스크립트|C|C[+][+]|Java|Java[Ss]cript|PHP"

#### Natural
KW_NTLNv = "독일어|북한어|스페인어|영어|이탈리아어|일본어|중국어|터키어|한국어"
...
KW_NTWf = "말|매듭|일단락|결론|이름|밥|한숨|소설|[보]?약|옷|표정|집|짝|아침"
KW_NTWv = "관계|노래|떼|[마]?무리|농사|미소|시|줄|[중]?죄"
...
KW_NTf = joinseq(KW_NTAf, KW_NTBf, KW_NTCf, KW_NTDf, KW_NTEf, KW_NTFf, KW_NTGf, KW_NTHf, KW_NTIf, KW_NTJf, KW_NTLf, KW_NTMf, KW_NTNf, KW_NTOf, KW_NTSf, KW_NTTf, KW_NTUf, KW_NTVf, KW_NTWf, KW_NTYf)
KW_NTv = joinseq(KW_NTAv, KW_NTBv, KW_NTCv, KW_NTDv, KW_NTEv, KW_NTFv, KW_NTGv, KW_NTHv,          KW_NTJv, KW_NTLv, KW_NTMv, KW_NTNv, KW_NTOv, KW_NTSv, KW_NTTv, KW_NTUv, KW_NTVv, KW_NTWv         )
KW_NT = joinseq(KW_NTf, KW_NTv)

## Nouns - Activity (can be verb with '-하다/-되다' or used in form of '~을/를 하다')
### Original (Korean + Hanja)
KW_NAOf1 = "곱"
KW_NAOf2 = "생각|보간|[가수증]감|동결|[가제]공|[건입출]국|공격|[연체]결|[구변]경|[총포]괄|집권|[접출]근|송금|지급|가늠|기능|[차판]단|[도전]달|[담배해]당|[가기운이작]동|[납취획]득|누락|[생중후]략|[노]력|[관마수제훈]련|[기수]록|소멸|[발변설소연작제조증]명|[괄주지]목|함몰|질문|고민|위반|[개선재폭]발|체벌|식별|[반중]복|구분|계산|[손연향]상|[검수탐]색|[발재]생|[분해]석|개선|[건개배연증해]설|[구생작완형]성|[계상접]속|[방소전]송|[기저]술|[연학]습|[발송수통]신|임신|[장파]악|[고보제]안|[계예해]약|선언|[수성영작]업|[구시재]연|[나배사]열|오열|오염|[반운촬투]영|[사응이작적통포허활]용|훈육|[기지]원|[대불]응|[방유]일|통일|[기도수출투]입|[동시조]작|[긴등성저출]장|경쟁|산적|[도발운회]전|조절|근접|[가개걱검결선설수조지추측한확]정|[마방]중|검증|감지|부진|[결매모수편]집|[도장부집탈]착|칭찬|제창|신청|[구압]축|[누도산수연인제진창추호]출|[계예]측|[강이]탈|[간선채]택|[면유입재]학|국한|방한|분할|포함|[결부취]합|[동미발비서시수운유연진]행|실행|시험|[구실재표]현|[반변]환|[계구기노]획"
...
KW_NAOv2 = "[인증추참평]가|[제탈]거|[공소전]개|설계|[경방]과|붕괴|[복연촉]구|[경대상연제표]기|[연증확]대|[시유주]도|계류|[고]려|신뢰|[완종치]료|[관격분수]리|처리|연마|구매|소모|근무|방미|[공거기]부|준비|[감공반발수식조퇴]사|[감기축]소|[감준회]수|[감실제출표]시|제어|[기부참]여|[소제]외|[긴소중필]요|[논유정주합회]의|투자|복제|참조|[유의주차폐]지|[기탑]재|대체|[갈성수탈편]취|[납배설위]치|검토|돌파|[부실]패|배포|발표|보호|[저전통]화|[분이]해"
...
KW_NAFf3 = "\\w{2}[깅닝딩링밍션싱징킹팅핑]|렌더링|로그인|컨트롤|컴파일"  
...
KW_NAFv3 = "트리거|업로드|릴리스|팔로우|플레이|마사지|마운트|임포트|카운트|테스트"
