...
## Thing(무정물)
### Abstract
KW_NTAf = "관측값|[공구]간|물건|채널|학년|모델|모듈|[능출]력|연령|프로그램|명산|걸음|전략|칼럼|입력|행렬|블록|그룹|[품항]목|논문|방법|성|특성|솔루션|애플리케이션|현실|직업|앱|차원|도메인|요인|타입|점|규정|[시지]점|수준|알고리즘|군집|층|토큰|스텝|시스템|패턴|팀|상품|튜플|토픽|모형"
KW_NTAv = "정도|메서드|스레드|코드|시류|메모리|처리|카테고리|리뷰|[요축]소|[난변인함]수|부동소수점수|분위수|순서|마이크로서비스|클래스|[단언]어|레이어|범위|차이|글자|일자|범주|[시절]차|아키텍처|피처|자체|가중치|관측치|배치|네트워크|에포크|태스크|데이터|벡터|학습 데이터|형태|세그먼트|이벤트|컴포넌트|크레이트|폰트|분포|변화|기회"
